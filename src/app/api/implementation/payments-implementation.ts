...
export const hasClientPaidInRange = async (
  clientId: number,
  startDate: Date,
  endDate: Date
) => {
...
export const getClientPaymentStatus = async (clientId: number) => {
  const now = new Date();
  const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
  const middleOfMonth = new Date(now.getFullYear(), now.getMonth(), 15);
  const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0);

  // Verificar si el cliente ha pagado en el rango del 1 al 15
  const hasPaidFirstHalf = await hasClientPaidInRange(
    clientId,
    startOfMonth,
    middleOfMonth
  );

  // Verificar si el cliente ha pagado en el rango del 16 al último día del mes
  const hasPaidSecondHalf = await hasClientPaidInRange(
    clientId,
    middleOfMonth,
    endOfMonth
  );
